...
def test_message_key_order_is_canonicalized_for_parent_matching():
    parent = _step(
        "parent",
        [1],
        [],
    ...
    parent_delta = _encode_against(parent, None)
    delta = _encode_against(child, parent)
    assert delta.parent_step_id == "parent"
    assert list(resolve_step_deltas([parent_delta, delta])[1].chat_completions[0]) == [
        "role",
        "content",
    ]
