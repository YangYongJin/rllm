...
class TestParseTinkerMessage:
    ...
    def test_invalid_json_arguments_remain_recoverable(self):
        assert _to_rllm_tool_calls([{"function": {"name": "calculator", "arguments": "{"}}]) == [
            RllmToolCall(name="calculator", arguments={"raw": "{"})
        ]
